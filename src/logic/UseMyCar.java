...
public class UseMyCar {
	...
	// thread fix for javafx printing
	...
	public UseMyCar(GameThread gameThread, CountDownLatch latch) {
	...
	public static void startGame(Car playerCar) {
		...
		Car myFirstCar = new Car("Other Car");

		// Create a car with a specified name: “Mazda” to be driven by a non-experienced
		// person.
		// This Car will start the race with an initial speed given by the player as a
		// parameter to the constructor
		// Let's take 20.
		Car mySecondCar = new Car();
		mySecondCar.setCarName(playerCar.getCarName());
...

		{
			...
			if (myFirstCar.getSpeedIncreaseStep() > 0) {
				Game.print(myFirstCar.getSpeedIncreaseStep() + " :: " + myFirstCar.getCarName() + " >>> : "
						+ myFirstCar.getCurrentSpeed() + "\t\t");
			}
			// if myFirstCar current speedIncreaseStep is negative (Deceleration - Slow
			// down)
			...
			if (mySecondCar.getSpeedIncreaseStep() > 0) {
				Game.println(mySecondCar.getSpeedIncreaseStep() + " :: " + mySecondCar.getCarName() + " >>> : "
			...
			try {
				...
				Game.println("there is an error");
			}
			i++;
		}
		Game.println();
		...
		if (myFirstCar.getCurrentSpeed() == mySecondCar.getCurrentSpeed()) {
			Game.println("\t\t" + "There is no winner for this Race ");
		} else if (myFirstCar.getCurrentSpeed() > mySecondCar.getCurrentSpeed()) {
			theWinner = myFirstCar.getCarName();
			Game.println("\t\t" + "The winner of this Race is :  " + theWinner);
		} else {
